...
export class SQLiteDocumentRepository implements DocumentRepository {
  ...
  public async findByLocalOriginalPath(localOriginalPath: string): Promise<Document | null> {
    const db = this.getDb();
    const row = db.prepare('SELECT * FROM documents WHERE localOriginalPath = ?').get(localOriginalPath);
    return row ? this.mapRow(row) : null;
  }
  ...
  public async listByFolderMappingId(folderMappingId: string): Promise<Document[]> {
    const db = this.getDb();
    const rows = db.prepare('SELECT * FROM documents WHERE folderMappingId = ?').all(folderMappingId);
    return rows.map((row) => this.mapRow(row));
  }
}
