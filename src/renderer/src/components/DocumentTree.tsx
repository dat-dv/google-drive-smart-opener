...
function FileIcon({ filename }: { filename: string }): React.JSX.Element {
  ...
  if (['xlsx', 'xls', 'csv'].includes(ext)) {
    return <span className="p-1 bg-emerald-500/10 text-emerald-400 rounded border border-emerald-500/20 text-[10px] font-bold shrink-0">XLS</span>
  }
  if (['docx', 'doc'].includes(ext)) {
    return <span className="p-1 bg-indigo-500/10 text-indigo-400 rounded border border-indigo-500/20 text-[10px] font-bold shrink-0">DOC</span>
  }
  if (['pptx', 'ppt'].includes(ext)) {
    return <span className="p-1 bg-orange-500/10 text-orange-400 rounded border border-orange-500/20 text-[10px] font-bold shrink-0">PPT</span>
  }
  if (ext === 'pdf') {
    return <span className="p-1 bg-red-500/10 text-red-400 rounded border border-red-500/20 text-[10px] font-bold shrink-0">PDF</span>
  }
  return (
 ...
 */
export function TreeNodeRow({ node, depth = 0, driveRootPath, expandedPaths, onToggle }: TreeNodeProps): React.JSX.Element {
  ...
  return (
    <div key={node.path || node.name} className="flex flex-col select-none">
      {/* Node Header Row */}
      <div
        onClick={(): void => { if (node.type === 'directory') onToggle(node.path) }}
        ...
        <div className="flex items-center gap-2 min-w-0">
          {node.type === 'directory' ? (
            <div className="flex items-center gap-1.5 shrink-0">
              <svg className={`w-3.5 h-3.5 text-slate-500 transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
              <svg className="w-4.5 h-4.5 text-amber-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
          ...
          <span className={`text-sm truncate ${node.type === 'directory' ? 'font-bold text-slate-300' : 'text-slate-200 font-mono text-xs'}`}>
            {node.name}
          </span>
        </div>
        ...
        {node.type === 'file' && node.document && (
          <div className="flex items-center gap-3 shrink-0 ml-4">
            <span className="text-[10px] text-slate-500 font-mono hidden md:inline">
              {node.document.metadata?.size ? `${(node.document.metadata.size / 1024).toFixed(1)} KB` : ''}
            </span>
            <span className="text-[10px] text-slate-500 hidden lg:inline">
              {node.document.lastOpened ? `Opened: ${new Date(node.document.lastOpened).toLocaleDateString()}` : ''}
            </span>
            ...
            {node.document.localOriginalPath && (
              <button
                onClick={(e): void => {
                  e.stopPropagation()
                  window.electron.ipcRenderer.invoke('dialog:open-file-path', node.document!.localOriginalPath)
                }}
...
export function DocumentTree({ docs, driveRootPath, expandedPaths, onToggle }: DocumentTreeProps): React.JSX.Element {
  if (docs.length === 0) {
    return (
